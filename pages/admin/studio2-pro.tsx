...
import { 
  ...
  MapPin, Phone, CreditCard, Power, X, Search, UserCheck, Truck, Crown, PackageSearch, Merge, CheckCircle2, Wifi, WifiOff, AlertCircle, Heart, RefreshCw, QrCode
} from 'lucide-react';
...
export default function App() {
  ...
  // 🔥 פונקציית נורמליזציה משופרת למניעת כפילויות בממשק
    ...
    const unsubCust = onSnapshot(query(collection(dbFS, 'customers'), limit(150)), (snap) => {
      ...
      raw.forEach((curr: any) => {
          ...
          if (!existing) {
            ...
            // העדפת המזהה המקורי של הטלפון על פני מזהי קבוצות מוזרים (LID)
  ...
  // 🔥 טעינת היסטוריה - סינכרון מהיר וחסין
  useEffect(() => {
    ...
    // מאזין להיסטוריה לפי המזהה המדויק שבו השרת משתמש
  ...
  const handleResetConnection = async () => {
    if (window.confirm("זה יבצע ריסטרט לשרת במשרד ויאלץ סריקת ברקוד חדשה. להמשיך?")) {
      ...
      try {
        ...
        alert("פקודת איפוס נשלחה. המתן לברקוד חדש.");
        setShowQrModal(true);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      alert("תקלה בשליחה. בדוק חיבור אינטרנט.");
    }
  };
  ...
  if (isPrinting) return (
    <div className="bg-white p-12 text-black font-serif min-h-screen overflow-auto" dir="rtl">
        <div className="max-w-4xl mx-auto border-[6px] border-double border-black p-10 bg-white shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-10 mb-10 bg-slate-50 p-8 border-2 border-black shadow-lg text-lg">
            <div className="space-y-2"><p className="text-xs font-black uppercase text-slate-500 underline underline-offset-4">יעד פרויקט</p><p className="text-3xl font-black">{editCrm.projectName || "כללי"}</p><p className="font-bold flex items-center gap-2"><MapPin size={18}/> {editCrm.projectAddress || "חסר כתובת"}</p></div>
    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-all duration-500 ${themeClass}`} dir="rtl">
        ...
        {selectedCustomer && activeTab === 'HUB' ? (
          <div className="flex-1 flex flex-col h-full">
            ...
            <div ref={scrollRef} className={`flex-1 overflow-y-auto p-12 flex flex-col gap-8 scroll-smooth no-scrollbar ${chatAreaBg}`} style={{backgroundImage: "url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')", backgroundBlendMode: theme === 'dark' ? 'soft-light' : 'overlay'}}>
              {messages.map((m, i) => (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} key={m.id || i} onClick={() => toggleSelection(m.id)} className={`flex flex-col max-w-[70%] ${m.type === 'in' ? 'self-start' : 'self-end items-end'} ${isSelectionMode ? 'cursor-pointer hover:scale-[1.02]' : ''} transition-transform`}>
                  <div className={`p-6 rounded-[2.2rem] shadow-2xl text-[14px] relative border leading-relaxed ${m.type === 'in' ? (theme === 'dark' ? 'bg-[#202c33] border-none text-slate-200 rounded-tr-none shadow-black/20' : 'bg-white border-none text-slate-800 rounded-tr-none') : 'bg-[#005c4b] text-white border-none rounded-tl-none shadow-emerald-500/40'}`}>
                    ...
                    <div className={`whitespace-pre-wrap font-bold leading-relaxed ${m.isSending ? 'opacity-50' : ''}`}>{m.text}</div>
              ...
              {isThinking && <div className="self-end bg-emerald-500/10 text-emerald-400 p-5 rounded-[2rem] flex items-center gap-5 border border-emerald-500/20 shadow-2xl animate-pulse font-black uppercase tracking-widest text-xs">JONI IS THINKING...</div>}
            </div>

            <footer className={`p-10 border-t z-20 ${sidebarBg}`}>
              {!isTrulyOnline && (
                  <div className="bg-red-500/10 text-red-500 p-4 rounded-2xl mb-6 text-center text-xs font-black border-2 border-dashed border-red-500/20 flex items-center justify-center gap-3 shadow-xl font-sans uppercase animate-pulse">
                    <WifiOff size={18}/> JONI Offline - Check office computer & Rescan QR
                  </div>
              )}
              <div className={`flex items-center gap-5 p-5 rounded-[3rem] border transition-all ${inputBg} shadow-2xl`}>
                <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="כתוב הודעה לניהול פרויקט (יכבה AI אוטומטית)..." className="flex-1 bg-transparent border-none outline-none text-base px-3 font-bold placeholder:text-slate-600 text-slate-800 dark:text-slate-200" />
      ...
      {!isMobile && selectedCustomer && (activeTab === 'HUB' || activeTab === 'CRM') && (
        <aside className={`w-[480px] flex flex-col border-r shrink-0 z-20 shadow-2xl ${sidebarBg}`}>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar bg-white dark:bg-transparent">
             <div className="flex flex-col items-center gap-8 pb-10 border-b border-white/5">
                ...
                <div className="text-center space-y-3 text-slate-800 dark:text-slate-100">
                   ...
                   <div className="flex justify-center gap-3">
                     <span className={`text-[10px] font-black px-5 py-2 rounded-full uppercase border ${isTrulyOnline ? 'text-emerald-500 bg-emerald-500/10 border-emerald-500/30' : 'text-red-500 bg-red-500/10 border-red-500/30'}`}>Real-ID: {selectedCustomer.id}</span>
                   </div>
                </div>
             </div>
        ...
        {showQrModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#020617]/95 backdrop-blur-2xl">
             <div className="bg-white rounded-[3.5rem] p-12 max-w-md w-full text-center shadow-[0_50px_100px_rgba(0,0,0,0.5)] border-4 border-amber-500/40 relative">
                ...
                <p className="text-slate-500 font-bold mb-10 leading-relaxed text-sm">השרת במשרד ממתין לסריקת ברקוד חדש.<br/><span className="text-amber-600">סרוק מהטלפון לחיבור הצינור.</span></p>
                <div className="bg-slate-50 p-8 rounded-[2.5rem] border-4 border-dashed border-slate-200 mb-10 aspect-square flex items-center justify-center min-h-[300px] shadow-inner relative">
                    {serverStatus.qr ? (
                        <img src={`https://api.qrserver.com/v1/create-qr-code/?size=350x350&data=${encodeURIComponent(serverStatus.qr)}`} className="w-full h-full shadow-2xl rounded-2xl border-8 border-white" alt="QR" />
                    ) : (
                ...
                <button onClick={handleResetConnection} className="w-full bg-red-600/10 text-red-600 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 border border-red-600/20 active:scale-95">בצע ריסטרט לשרת במשרד</button>
             </div>
          </motion.div>
        )}
      </AnimatePresence>

      {theme === 'dark' && <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden"><div className="absolute top-[-10%] right-[-5%] w-[800px] h-[800px] bg-emerald-500/5 rounded-full blur-[180px]"></div><div className="absolute bottom-[-10%] left-[-5%] w-[800px] h-[800px] bg-blue-500/5 rounded-full blur-[150px]"></div></div>}
    </div>
  );
}
