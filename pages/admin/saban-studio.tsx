      ...
      <section className="p-4 bg-slate-900/40 border-b border-slate-800 shrink-0">
        <div className="max-w-5xl mx-auto flex gap-2">
          <div className="relative flex-1">
            <input 
              className="w-full p-4 pr-11 bg-slate-800 border border-slate-700 rounded-2xl outline-none focus:border-blue-500 font-bold text-sm text-white"
          ...
          <button onClick={autoInject} className="bg-blue-600 p-4 rounded-2xl active:scale-90 transition-transform">
            {loading ? <Loader2 className="animate-spin" size={20}/> : <Zap size={20}/>}
          </button>
          <button onClick={openManualEntry} className="bg-slate-700 p-4 rounded-2xl active:scale-90 transition-transform">
            <Plus size={20}/>
          </button>
        </div>
      </section>
          ...
          {items.map(item => (
            <div key={item.id} className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] overflow-hidden flex flex-col shadow-xl">
              <div className="h-40 relative">
                ...
                <div className="absolute top-4 right-4 bg-blue-600 px-3 py-1 rounded-lg text-[9px] font-black uppercase shadow-lg">
                  {item.sku}
                </div>
              </div>
              <div className="p-6 flex-1 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-700/30">
                    <span className="text-[8px] text-slate-500 block font-black mb-1 uppercase tracking-widest"><Droplets size={10} className="inline ml-1" /> ייבוש</span>
                    <span className="text-[11px] font-bold text-blue-400">{item.dry_time || '-'}</span>
                  </div>
                  <div className="bg-slate-800/40 p-3 rounded-2xl border border-slate-700/30">
                    <span className="text-[8px] text-slate-500 block font-black mb-1 uppercase tracking-widest"><Package size={10} className="inline ml-1" /> כיסוי</span>
                ...
                <div className="mt-auto pt-5 border-t border-slate-800 flex justify-between items-center">
                  ...
                  <button onClick={() => {setEditingProduct(item); setIsModalOpen(true);}} className="p-3 bg-slate-800 rounded-2xl text-slate-400 hover:text-white transition-all">
                    <Edit2 size={20}/>
                  </button>
                </div>
              </div>
      ...
      {isModalOpen && editingProduct && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-slate-900 animate-in slide-in-from-bottom">
          <header className="p-5 border-b border-slate-800 flex justify-between items-center">
            <h2 className="text-xl font-black italic tracking-tighter">עריכת מוצר <span className="text-blue-500">Saban AI</span></h2>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
            ...
            <textarea 
              className="w-full p-5 bg-slate-800 rounded-[1.5rem] outline-none font-bold text-sm text-white min-h-[140px]"
          ...
          <div className="p-4 border-t border-slate-800">
            <button onClick={saveToDB} className="w-full bg-blue-600 text-white py-5 rounded-[2rem] font-black text-lg flex items-center justify-center gap-3 active:scale-95 transition-all shadow-2xl">
