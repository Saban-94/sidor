...
export default function SabanOSMaster() {
  ...
  const startSim = (user: any) => {
    ...
    setChatMessages([
      { role: 'system', content: `איתחול סימולציה. ה-DNA הכללי הוטמע. הנחיה ספציפית: ${user.brain_dna_notes || 'אין'}` },
  ...
  const sendSimMsg = () => {
    ...
    setTimeout(() => {
      setChatMessages(prev => [...prev, { role: 'assistant', content: `בהתאם ל-DNA שלי ולמידע עליך, אני עונה על: ${currentInput}` }]);
    }, 800);
  };
      ...
      <main className="p-6 grid grid-cols-1 xl:grid-cols-3 gap-8 max-w-[1600px] mx-auto">
        <div className="xl:col-span-2 space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-black flex items-center gap-2"><Users className="text-emerald-500" /> מאגר משתמשים</h2>
                  ...
                  <tr key={c.id} onClick={() => startSim(c)} className={`hover:bg-emerald-50/30 transition-all cursor-pointer ${selectedUserForChat?.id === c.id ? 'bg-emerald-50/80 border-r-4 border-r-emerald-500' : ''}`}>
                    ...
                    <td className="p-5">
                      <div className="flex flex-wrap gap-1">
                        {c.family_relation && <span className="bg-blue-50 text-blue-600 text-[10px] px-2 py-0.5 rounded-full font-bold">{c.family_relation} לרמי</span>}
                    ...
                    <td className="p-5 text-center" onClick={e => e.stopPropagation()}>
                      <button onClick={() => {
                        ...
                        alert("הועתק!");
                      }} className="p-2.5 bg-white border rounded-xl text-slate-400 hover:text-blue-500"><Copy size={18}/></button>
                    </td>
        ...
        {/* Simulator Area */}
        <div className="xl:col-span-1 flex justify-center h-fit sticky top-28">
           <div className="w-[300px] h-[600px] bg-black rounded-[50px] border-[10px] border-black shadow-2xl relative overflow-hidden flex flex-col p-2">
              ...
              {selectedUserForChat ? (
                ...
                <div className="flex-1 bg-slate-900 rounded-[30px] flex flex-col items-center justify-center p-6 text-center text-slate-600 gap-4">
                  <Smartphone size={40}/>
                  <div className="text-[10px] font-bold">לחץ על משתמש בטבלה לסימולציה</div>
                </div>
              )}
           </div>
        </div>
        ...
        {activeTab === 'DNA' && (
          <div className="xl:col-span-2 max-w-2xl mx-auto w-full">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white border rounded-[2.5rem] p-8 shadow-sm space-y-6">
              <h2 className="text-2xl font-black flex items-center gap-3"><BrainCircuit size={28} className="text-emerald-500"/> ליבת המוח</h2>
              <textarea 
                className="w-full h-96 bg-slate-50 border rounded-2xl p-5 outline-none focus:ring-2 focus:ring-emerald-500/10 text-sm leading-relaxed"
                value={botConfig.instructions}
                onChange={(e) => setBotConfig({...botConfig, instructions: e.target.value})}
                placeholder="הכנס DNA כללי..."
              />
              <button onClick={async () => {
                ...
                setLoading(false);
                alert("ה-DNA עודכן!");
              }} className="w-full bg-slate-950 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2">
                {loading ? <Activity className="animate-spin"/> : <Save size={20}/>} שמור והטמע
              </button>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm bg-slate-900/40">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-2xl rounded-[2.5rem] p-8 shadow-2xl">
              <h3 className="text-xl font-black mb-6">עריכת פרופיל משתמש</h3>
              <form onSubmit={handleSaveUser} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Input name="phone" label="טלפון" defaultValue={editingUser?.phone} required />
                <Input name="email" label="אימייל" defaultValue={editingUser?.email} type="email" />
                <Input name="family_relation" label="קשר משפחתי" defaultValue={editingUser?.family_relation} />
                <Input name="hobbies" label="תחביבים" defaultValue={editingUser?.hobbies} />
                <div className="md:col-span-2 space-y-1">
                  <label className="text-[10px] font-black text-emerald-600 uppercase">הנחיה ספציפית</label>
                  <textarea name="brain_dna_notes" defaultValue={editingUser?.brain_dna_notes} className="w-full h-24 p-3 bg-slate-50 border rounded-2xl text-xs outline-none focus:ring-2 focus:ring-emerald-200" />
                </div>
                <button type="submit" className="md:col-span-2 bg-slate-900 text-white font-black py-4 rounded-2xl shadow-lg mt-4">{editingUser ? 'עדכן' : 'צור'}</button>
              </form>
            </motion.div>
          </div>
        )}
...
const Input = ({ label, name, ...props }: any) => (
  <div className="space-y-1">
    ...
    <input name={name} {...props} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-emerald-200 transition-all" />
  </div>
);  // --- סימולטור ---
  const [selectedUserForChat, setSelectedUserForChat] = useState<any>(null);
  const [chatMessages, setChatMessages] = useState<any[]>([]);
