...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 1. טעינת לקוחות עם איחוד בזמן אמת (תיקון TypeScript לבילד)
  ...
  const saveCustomerCard = async () => {
    ...
    try {
      ...
      console.error('❌ שגיאה בשמירת כרטיס:', err.message);
    } finally {
  ...
  const handleMergeManual = async () => {
    const targetPhone = prompt("הכנס מספר טלפון לאיחוד (972...):");
    if (!targetPhone || !selectedCustomer) return;
    setIsSaving(true);
    try {
      await setDoc(doc(dbFS, 'customers', targetPhone), { ...selectedCustomer, id: targetPhone, lastUpdated: serverTimestamp() }, { merge: true });
      alert("אוחד בהצלחה.");
    } catch (err: any) {
      console.error('❌ שגיאה באיחוד:', err.message);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="bg-white p-12 text-black font-serif min-h-screen overflow-auto" dir="rtl">
        <div className="max-w-4xl mx-auto border-[6px] border-double border-black p-10 shadow-2xl">
          <div className="flex justify-between items-center border-b-4 border-black pb-8 mb-8">
            <div>
              <h1 className="text-5xl font-black italic tracking-tighter">ח. סבן - הזמנה</h1>
          ...
          <div className="grid grid-cols-2 gap-10 mb-10 bg-slate-50 p-6 border-2 border-black shadow-lg">
            <div><p className="text-xs font-black uppercase underline">פרויקט</p><p className="text-2xl font-black">{editCrm.projectName || "פרויקט כללי"}</p><p>{editCrm.projectAddress}</p></div>
            <div className="text-left"><p className="text-xs font-black uppercase underline">לקוח</p><p className="text-xl font-bold">קומקס: {editCrm.comaxId}</p><p>{editCrm.contactName}</p></div>
          </div>
          <div className="border-2 border-black min-h-[500px] flex flex-col">
            <div className="bg-black text-white p-3 flex justify-between font-black text-lg"><span>תיאור מהצ'אט</span><span>כמות</span></div>
          ...
          <div className="mt-16 flex justify-between items-end pt-10 border-t-4 border-black italic">
            <p>חתימת מנהל: ________________</p><p>Saban Operational Hub</p>
          </div>
        </div>
        <div className="fixed bottom-10 left-10 flex gap-6 no-print">
          <button onClick={() => setIsPrinting(false)} className="bg-slate-900 text-white px-8 py-4 rounded-3xl font-black">חזרה</button>
    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-all duration-500 ${themeClass}`} dir="rtl">
          ...
          <div className="flex-1 flex flex-col h-full">
            <header className={`h-24 flex items-center justify-between px-12 border-b z-20 ${sidebarBg}`}>
              <div className="flex items-center gap-6">
                ...
                <div><h2 className="font-black text-2xl italic tracking-tighter leading-none">{editCrm.projectName || selectedCustomer.name}</h2><p className="text-[11px] font-bold text-slate-500 mt-2 uppercase tracking-[0.3em] flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>{selectedCustomer.id} | JONI UNIFIED</p></div>
              </div>
              <div className="flex items-center gap-6">
            ...
            <footer className={`p-10 border-t z-20 ${sidebarBg}`}>
              <div className={`flex items-center gap-5 p-5 rounded-[2.8rem] border transition-all ${inputBg} shadow-2xl`}>
                <button className="p-3 text-slate-500 hover:text-emerald-500 transition-all"><ImageIcon size={28}/></button>
                <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="כתוב למנהל הפרויקט..." className="flex-1 bg-transparent border-none outline-none text-base px-3 font-bold" />
                <button onClick={handleSend} className="w-16 h-16 bg-emerald-600 text-white rounded-[2rem] flex items-center justify-center shadow-2xl active:scale-90 transition-all"><Send size={28} className="transform rotate-180" /></button>
              </div>
            </footer>
          </div>
        ) : activeTab === 'FLOW' ? (
      ...
      {!isMobile && selectedCustomer && (activeTab === 'HUB' || activeTab === 'CRM') && (
        <aside className={`w-[480px] flex flex-col border-r shrink-0 z-20 shadow-2xl ${sidebarBg}`}>
          <header className="p-8 border-b border-inherit bg-blue-600/5 flex justify-between items-center"><h2 className="font-black text-sm uppercase tracking-widest flex items-center gap-3"><UserCog size={24} className="text-blue-500"/> כרטיס ניהול פרויקט</h2><button onClick={handleMergeManual} className="bg-indigo-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-indigo-500 transition-all"><Merge size={12}/> איחוד ידני</button></header>
          <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
             <div className="flex flex-col items-center gap-8 pb-10 border-b border-white/5">
                ...
                <div className="text-center space-y-3"><h3 className="text-3xl font-black italic tracking-tighter">{editCrm.contactName || selectedCustomer.name}</h3><div className="flex justify-center gap-3"><span className="text-[10px] font-black text-emerald-500 bg-emerald-500/10 px-5 py-2 rounded-full uppercase border border-emerald-500/30">מנהל פרויקט VIP</span></div></div>
             </div>
             ...
             <button onClick={saveCustomerCard} disabled={isSaving} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-6 rounded-[2.5rem] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-5 mt-6 uppercase tracking-widest">{isSaving ? <Activity size={24} className="animate-spin"/> : <><Save size={24}/> Sync & Unify Card</>}</button>
          </div>
        </aside>
      )}
