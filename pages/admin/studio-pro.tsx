...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 1. טעינת לקוחות עם איחוד לוגי (למניעת כפל ברשימה)
    const unsubCust = onSnapshot(query(collection(dbFS, 'customers'), limit(150)), (snap) => {
      ...
      raw.forEach((curr: any) => {
          ...
          // שמירה על הגרסה עם השם או התמונה
  ...
  const handleSend = async () => {
    ...
    if (isAiActive) setIsAiActive(false); // השתלטות ידנית
  ...
  // 🔥 איחוד ידני - מוחק את הישן ומעביר היסטוריה (מעלי הקבוצה לעלי הטלפון)
    ...
    <div className="bg-white p-12 text-black font-serif min-h-screen overflow-auto" dir="rtl">
        <div className="max-w-4xl mx-auto border-[6px] border-double border-black p-10 shadow-2xl">
          ...
          <div className="mt-16 flex justify-between items-end pt-10 border-t-4 border-black italic text-sm">
            <div className="text-center space-y-2"><div className="w-48 border-b-2 border-black mx-auto h-12"></div><p className="font-black text-xs uppercase text-slate-600">חתימת מנהל פרויקט</p></div>
            <div className="text-center space-y-2"><div className="w-48 border-b-2 border-black mx-auto h-12"></div><p className="font-black text-xs uppercase text-slate-600">אישור מחסן ח. סבן</p></div>
          </div>
        </div>
        ...
        {selectedCustomer && activeTab === 'HUB' ? (
          <div className="flex-1 flex flex-col h-full">
            ...
            <div ref={scrollRef} className={`flex-1 overflow-y-auto p-12 flex flex-col gap-8 scroll-smooth no-scrollbar ${chatAreaBg}`} style={{backgroundImage: "url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')", backgroundBlendMode: theme === 'dark' ? 'soft-light' : 'overlay'}}>
              ...
              {isThinking && <div className="self-end bg-emerald-500/10 text-emerald-400 p-5 rounded-[2rem] flex items-center gap-5 border border-emerald-500/20 shadow-2xl animate-pulse"><Activity size={24} className="animate-spin" /><span className="text-sm font-black uppercase tracking-widest">JONI AI FORMULATING...</span></div>}
            </div>
            ...
            <footer className={`p-10 border-t z-20 ${sidebarBg}`}>
              <div className={`flex items-center gap-5 p-5 rounded-[2.8rem] border transition-all ${inputBg} shadow-2xl`}>
                ...
                <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="כתוב למנהל הפרויקט (השתלטות ידנית)..." className="flex-1 bg-transparent border-none outline-none text-base px-3 font-bold placeholder:text-slate-600" />
        ...
        <aside className={`w-[480px] flex flex-col border-r shrink-0 z-20 shadow-2xl ${sidebarBg}`}>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
             <div className="flex flex-col items-center gap-8 pb-10 border-b border-white/5">
                ...
                <div className="text-center space-y-3"><h3 className="text-3xl font-black italic tracking-tighter">{editCrm.contactName || selectedCustomer.name}</h3><span className="text-[10px] font-black text-emerald-500 bg-emerald-500/10 px-5 py-2 rounded-full uppercase border border-emerald-500/30">זהות מאוחדת: {normalizeId(selectedCustomer.id)}</span></div>
             </div>
             ...
             {/* 🔥 תיקון שגיאת בילד: שימוש ב-saveCustomerCard במקום saveCrm */}
