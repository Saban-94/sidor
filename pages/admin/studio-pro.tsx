...
// --- אתחול Firebase (בטוח מול Vercel) ---
...
export default function SabanUnifiedHub() {
  ...
  // --- 🔥 לוגיקת איחוד וזיהוי (Normalization) ---
  const normalizeId = (id: string) => {
      if (!id) return '';
      // אם זה מזהה קבוצה (מכיל - או @g.us), נשאיר אותו כרגע, אם זה טלפון - ננקה ל-9 ספרות
      const digits = id.replace(/\D/g, '');
      return digits.length >= 9 ? digits.slice(-9) : id;
  };
  ...
  useEffect(() => {
    ...
    // 1. טעינת לקוחות עם איחוד אוטומטי ברמת התצוגה
    const unsubCust = onSnapshot(query(collection(dbFS, 'customers'), limit(150)), (snap) => {
      ...
      raw.forEach((curr: any) => {
          ...
          // שמירה על הגרסה העדכנית ביותר או זו עם הכי הרבה נתונים
  ...
  // טעינת היסטוריה - מוודא סינכרון מלא
  useEffect(() => {
    ...
    // מאזין להיסטוריה של המזהה הספציפי
  ...
  const handleSend = async () => {
    ...
    if (isAiActive) setIsAiActive(false); // שליטה עוינת: השתקה אוטומטית של ה-AI
  ...
  const handlePhotoUpdate = async () => {
    const newUrl = prompt("הכנס לינק לתמונת פרופיל חדשה (URL):", editCrm.photo);
  ...
  const handleMergeManual = async () => {
    ...
    const confirmMerge = window.confirm(`האם לאחד את הנתונים לתוך המספר ${targetPhone}?`);
    if (confirmMerge) {
      setIsSaving(true);
      ...
      alert("אוחד בהצלחה. המערכת תזהה מעכשיו את המשתמש תחת המספר הזה.");
      setIsSaving(false);
    }
  };
  ...
  if (isPrinting) return (
    <div className="bg-white p-12 text-black font-serif min-h-screen overflow-auto" dir="rtl">
        <div className="max-w-4xl mx-auto border-[6px] border-double border-black p-10 shadow-2xl">
          ...
          <div className="border-2 border-black min-h-[500px] flex flex-col shadow-inner">
            ...
            <div className="p-6 space-y-6 flex-1">
              ...
              {selectedMsgIds.length === 0 && <p className="m-auto text-slate-400 italic">לא נבחרו הודעות להדפסה.</p>}
            </div>
          </div>
          ...
          <div className="mt-16 flex justify-between items-end pt-10 border-t-4 border-black italic">
            ...
            <div className="text-center opacity-60 text-xs">הופק ע"י Saban HUB Command</div>
        ...
        <div className="fixed bottom-10 left-10 flex gap-6 no-print">
          <button onClick={() => setIsPrinting(false)} className="bg-slate-900 text-white px-8 py-4 rounded-3xl font-black shadow-2xl hover:scale-105 transition-all">חזור לצ'אט</button>
        ...
        <aside className={`w-85 flex flex-col border-l shrink-0 z-30 ${sidebarBg}`}>
          <header className="p-7 border-b border-inherit bg-emerald-500/5 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                ...
                <span className="text-[9px] bg-emerald-500 text-white px-2 py-0.5 rounded-full font-black animate-pulse uppercase tracking-tighter">Live System</span>
            </div>
      ...
      <main className="flex-1 relative flex flex-col bg-transparent z-10" style={{ backgroundImage: theme === 'dark' ? 'radial-gradient(#1e293b 0.5px, transparent 0.5px)' : 'radial-gradient(#cbd5e1 0.5px, transparent 0.5px)', backgroundSize: '32px 32px' }}>
        ...
        {selectedCustomer && activeTab === 'HUB' ? (
          <div className="flex-1 flex flex-col h-full">
            <header className={`h-24 flex items-center justify-between px-12 border-b z-20 ${sidebarBg}`}>
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-[11px] font-bold text-slate-500 mt-2 uppercase tracking-[0.3em] flex items-center gap-2">
                    ...
                    {selectedCustomer.id} | JONI UNIFIED CORE
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl font-black text-xs transition-all border ${isSelectionMode ? 'bg-orange-500 border-orange-400 text-white shadow-lg shadow-orange-500/20' : 'bg-slate-500/10 border-slate-500/20 text-slate-500'}`}
                >
                  <Activity size={16} /> {isSelectionMode ? 'סימון הודעות' : 'בחירת פריטים'}
                </button>
            ...
            <div ref={scrollRef} className={`flex-1 overflow-y-auto p-12 flex flex-col gap-8 scroll-smooth no-scrollbar ${theme === 'dark' ? 'bg-[#020617]' : 'bg-[#e5ddd5]'}`} style={{backgroundImage: "url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')", backgroundBlendMode: theme === 'dark' ? 'soft-light' : 'overlay'}}>
              {messages.map((m, i) => (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} key={m.id || i} 
                  ...
                  <div className={`p-6 rounded-[2.2rem] shadow-2xl text-[14px] relative border leading-relaxed ${m.type === 'in' ? (theme === 'dark' ? 'bg-[#202c33] border-none text-slate-200 rounded-tr-none' : 'bg-white border-none text-slate-800 rounded-tr-none') : 'bg-[#005c4b] text-white border-none rounded-tl-none shadow-emerald-500/40'}`}>
                    ...
                    <div className="text-[9px] font-black opacity-30 mb-2 flex items-center gap-2 uppercase tracking-tighter">
                      {m.source === 'group' ? <Users size={12}/> : <Smartphone size={12}/>} {m.source === 'group' ? 'קבוצה' : 'אישי'}
                    </div>
          ...
          <div className="m-auto flex flex-col items-center gap-10 opacity-20 group">
             ...
             <p className="text-sm font-bold tracking-[0.8em] text-center uppercase">Secure Operating System v5.0</p>
          </div>
        )}
      </main>
      ...
      {!isMobile && selectedCustomer && (activeTab === 'HUB' || activeTab === 'CRM') && (
        <aside className={`w-[480px] flex flex-col border-r shrink-0 z-20 shadow-2xl ${sidebarBg}`}>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
             ...
             <button onClick={saveCrm} disabled={isSaving} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-6 rounded-[2.5rem] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-5 mt-6 uppercase tracking-widest text-base">{isSaving ? <Activity size={24} className="animate-spin"/> : <><Save size={24}/> Update & Sync Card</>}</button>
          </div>
        </aside>
      )}
...
const toggleMessageSelection = (id: string) => { /* Placeholder in component scope */ };
