...
export default function App() {
  ...
  const handlePhotoUpdate = async () => {
    const newUrl = prompt("הכנס לינק לתמונה חדשה:", editCrm.photo);
  ...
  // 🔥 איחוד משתמשים אמיתי (מוחק את הישן ומעביר היסטוריה)
  const handleMergeManual = async () => {
    ...
    if (window.confirm(`מבצע איחוד סופי: מעביר את כל ההיסטוריה של ${selectedCustomer.id} ל-${targetPhone} ומוחק את המזהה הישן. לאשר?`)) {
      setIsSaving(true);
      try {
        ...
        await batch.commit();
        alert("האיחוד הושלם בהצלחה. המשתמש הישן נמחק.");
        setSelectedCustomer(null);
      } catch (err: any) { alert("שגיאה באיחוד: " + err.message); } finally { setIsSaving(false); }
    }
  };
    ...
    <div className="bg-white p-12 text-black font-serif min-h-screen" dir="rtl">
        <div className="max-w-4xl mx-auto border-[6px] border-double border-black p-10 shadow-2xl">
          ...
          <div className="border-2 border-black min-h-[500px] flex flex-col shadow-inner">
            <div className="bg-black text-white p-3 flex justify-between font-black text-lg"><span>תיאור שנבחר מהצ'אט</span><span className="w-32 text-center">כמות</span></div>
            <div className="p-6 space-y-6 flex-1">
              {messages.filter(m => selectedMsgIds.includes(m.id)).map((m, i) => (
                <div key={i} className="flex justify-between border-b-2 border-slate-200 pb-4 items-center">
        ...
        <div className="fixed bottom-10 left-10 flex gap-6 no-print">
          <button onClick={() => setIsPrinting(false)} className="bg-slate-900 text-white px-8 py-4 rounded-3xl font-black shadow-2xl">חזור</button>
          <button onClick={() => window.print()} className="bg-emerald-600 text-white px-8 py-4 rounded-3xl font-black shadow-2xl flex items-center gap-2"><Printer size={22}/> הדפס</button>
        </div>
    </div>
  );
    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-all duration-500 ${themeClass}`} dir="rtl">
        ...
        <aside className={`w-85 flex flex-col border-l shrink-0 z-30 ${sidebarBg}`}>
          <header className="p-7 border-b border-inherit bg-emerald-500/5 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <h2 className="font-black text-sm uppercase tracking-widest flex items-center gap-2"><MessageCircle size={18} className="text-emerald-500"/> פניות JONI</h2>
                <span className="text-[9px] bg-emerald-500 text-white px-2 py-0.5 rounded-full font-black animate-pulse">LIVE</span>
            </div>
            <div className={`relative ${inputBg} rounded-xl overflow-hidden shadow-inner`}>
                ...
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="חיפוש מנהל פרויקט..." className="w-full bg-transparent p-2.5 pr-10 text-xs border-none outline-none font-bold" />
            </div>
          </header>
      ...
      <main className="flex-1 relative flex flex-col bg-transparent z-10" style={{ backgroundImage: theme === 'dark' ? 'radial-gradient(#1e293b 0.5px, transparent 0.5px)' : 'radial-gradient(#cbd5e1 0.5px, transparent 0.5px)', backgroundSize: '32px 32px' }}>
        {selectedCustomer && activeTab === 'HUB' ? (
          <div className="flex-1 flex flex-col h-full">
            <header className={`h-24 flex items-center justify-between px-12 border-b z-20 ${sidebarBg}`}>
              <div className="flex items-center gap-6">
                ...
                <div><h2 className="font-black text-2xl italic tracking-tighter leading-none">{editCrm.projectName || selectedCustomer.name}</h2><p className="text-[11px] font-bold text-slate-500 mt-2 uppercase tracking-[0.3em] flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>{normalizeId(selectedCustomer.id)} | UNIFIED HUB</p></div>
              </div>
            ...
            <footer className={`p-10 border-t z-20 ${sidebarBg}`}>
              <div className={`flex items-center gap-5 p-5 rounded-[2.8rem] border transition-all ${inputBg} shadow-2xl`}>
                ...
                <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="כתוב למנהל הפרויקט..." className="flex-1 bg-transparent border-none outline-none text-base px-3 font-bold placeholder:text-slate-600" />
          ...
          <div className="flex-1 overflow-y-auto p-16 flex flex-col gap-12">
            <header className="flex justify-between items-center"><div><h1 className="text-5xl font-black italic tracking-tighter uppercase">AI <span className="text-blue-500">Studio</span></h1><p className="text-sm font-bold opacity-60 mt-2 uppercase tracking-widest">תכנות המוח המרכזי</p></div><div className="flex gap-4"><button onClick={() => setNodes([...nodes, { id: `BNCH_${Date.now()}`, title: 'ענף חדש', trigger: '', prompt: '' }])} className="bg-blue-600 text-white px-10 py-5 rounded-[1.8rem] font-black text-sm shadow-2xl active:scale-95 transition-all flex items-center gap-3"><Plus size={24}/> ענף חדש</button><button onClick={async () => { setIsSaving(true); await setDoc(doc(dbFS, 'system', 'bot_flow_config'), { nodes, globalDNA }, { merge: true }); setTimeout(()=>setIsSaving(false), 800); }} className="bg-slate-900 text-white px-10 py-5 rounded-[1.8rem] font-black text-sm shadow-2xl flex items-center gap-3 transition-all">{isSaving ? <Activity className="animate-spin"/> : <><Save size={24}/> שמור עץ LIVE</>}</button></div></header>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">{nodes.map(node => (
                <div key={node.id} className={`p-10 rounded-[3rem] border relative group hover:shadow-2xl transition-all ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200 shadow-xl'}`}><button onClick={() => setNodes(nodes.filter(n => n.id !== node.id))} className="absolute -left-2 -top-2 w-12 h-12 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-2xl hover:scale-110 z-10"><Trash2 size={20}/></button><div className="grid grid-cols-2 gap-8 mb-8"><div><label className="text-[10px] font-black opacity-50 uppercase tracking-widest block mb-2">שם הענף</label><input value={node.title} onChange={e => setNodes(nodes.map(n => n.id === node.id ? {...n, title: e.target.value} : n))} className={`w-full p-4 rounded-2xl text-xs font-black outline-none border-2 focus:border-blue-500 transition-all ${inputBg}`} /></div><div><label className="text-[10px] font-black opacity-50 uppercase tracking-widest block mb-2">טריגר</label><input value={node.trigger} onChange={e => setNodes(nodes.map(n => n.id === node.id ? {...n, trigger: e.target.value} : n))} className={`w-full p-4 rounded-2xl text-xs font-mono text-blue-400 outline-none border-2 focus:border-blue-500 transition-all ${inputBg}`} placeholder="למשל: 1" /></div></div><textarea value={node.prompt} onChange={e => setNodes(nodes.map(n => n.id === node.id ? {...n, prompt: e.target.value} : n))} className={`w-full h-40 p-5 rounded-[1.8rem] text-[12px] font-medium leading-relaxed resize-none outline-none border-2 focus:border-blue-500 transition-all ${inputBg}`} placeholder="מה ה-AI עונה בשלב זה..." /></div>
            ))}</div>
          </div>
        ) : (
          <div className="m-auto flex flex-col items-center gap-10 opacity-20 group"><div className="relative"><MessageCircle size={200} /><Bot size={70} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-emerald-500 animate-bounce" /></div><h2 className="text-6xl font-black italic tracking-tighter uppercase text-center leading-tight">SABAN HUB<br/>UNIFIED COMMAND</h2><p className="text-sm font-bold tracking-[0.8em] text-center uppercase">Secure Operating System v5.2</p></div>
        )}
      </main>
      ...
      {!isMobile && selectedCustomer && (activeTab === 'HUB' || activeTab === 'CRM') && (
        <aside className={`w-[480px] flex flex-col border-r shrink-0 z-20 shadow-2xl ${sidebarBg}`}>
          <header className="p-8 border-b border-inherit bg-blue-600/5 flex justify-between items-center"><h2 className="font-black text-sm uppercase tracking-widest flex items-center gap-3"><UserCog size={24} className="text-blue-500"/> ניהול זהות</h2><button onClick={handleMergeManual} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[11px] font-black uppercase flex items-center gap-2 hover:bg-indigo-500 transition-all shadow-lg"><Merge size={14}/> איחוד ומחיקה</button></header>
          <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
             ...
             <button onClick={saveCrm} disabled={isSaving} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-6 rounded-[2.5rem] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-5 mt-6 uppercase tracking-widest text-base">{isSaving ? <Activity size={24} className="animate-spin"/> : <><Save size={24}/> Sync & Unify Card</>}</button>
          </div>
        </aside>
      )}
...
const themeClass = ""; // Logic in App
const sidebarBg = ""; // Logic in App
const inputBg = ""; // Logic in App
