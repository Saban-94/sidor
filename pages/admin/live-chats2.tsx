...
import { 
  ...
  MapPin, Phone, CreditCard, Power, X, Search, UserCheck, Truck, Crown, PackageSearch, Merge, CheckCircle2, Wifi, WifiOff, AlertCircle, Heart, RefreshCw, QrCode, Trash
} from 'lucide-react';
...
export default function App() {
  ...
  // 🔥 לוגיקת נורמליזציה: הופכת כל ID לזהות של 9 ספרות אחרונות (מפתח האיחוד)
    ...
    const unsubCust = onSnapshot(query(collection(dbFS, 'customers'), orderBy('lastUpdated', 'desc'), limit(150)), (snap) => {
      ...
      raw.forEach((curr: any) => {
          ...
          if (!existing) {
            ...
            // הלחמה: אם יש כרטיס עם יותר מידע (שם/תמונה), הוא הופך לראשי
  ...
  // טעינת היסטוריה וסנכרון CRM ללקוח
  useEffect(() => {
  ...
  // 🔥 פונקציית האיחוד המוחלט - מעבירה היסטוריה ומוחקת כפילות פיזית מה-DB
  const handleMergeManual = async () => {
    ...
    if (window.confirm(`⚠️ אזהרה: כל ההיסטוריה של ${selectedCustomer.id} תועבר ל-${targetPhone} והמזהה הישן יימחק לצמיתות. לאשר?`)) {
      ...
      try {
        ...
        await batch.commit();
        alert("איחוד זהויות הושלם. הכפילות נמחקה מהמאגר.");
  ...
  return (
    <div className={`flex h-screen font-sans overflow-hidden transition-all duration-500 ${themeClass}`} dir="rtl">
      ...
      {/* 2. רשימת זהויות מאוחדת (WhatsApp Style) */}
        ...
        <aside className={`w-96 flex flex-col border-l shrink-0 z-30 ${sidebarBg}`}>
          <header className="p-7 border-b border-inherit bg-emerald-500/5 flex flex-col gap-5">
            <div className="flex justify-between items-center text-slate-800 dark:text-slate-100">
                ...
                <span className={`text-[9px] px-2 py-0.5 rounded-full font-black uppercase ${isTrulyOnline ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white animate-pulse'}`}>{isTrulyOnline ? 'Sync OK' : 'Offline'}</span>
            </div>
        ...
        <aside className={`w-[500px] flex flex-col border-r shrink-0 z-20 shadow-2xl ${sidebarBg}`}>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar bg-white dark:bg-transparent">
             ...
             <div className="space-y-10">
                <div className="space-y-3"><label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2"><CreditCard size={14} className="text-emerald-500"/> מספר לקוח בקומקס</label><input value={editCrm.comaxId} onChange={e => setEditCrm((prev:any)=>({...prev, comaxId: e.target.value}))} className={`w-full p-6 rounded-[2rem] text-lg font-black outline-none border-2 transition-all ${inputBg} focus:border-emerald-500 shadow-2xl text-slate-800 dark:text-slate-100`} /></div>
                <div className="space-y-3"><label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2"><Building size={14} className="text-blue-500"/> שם פרויקט / חברה</label><input value={editCrm.projectName} onChange={e => setEditCrm((prev:any)=>({...prev, projectName: e.target.value}))} className={`w-full p-6 rounded-[2rem] text-lg font-black outline-none border-2 transition-all ${inputBg} focus:border-blue-500 shadow-2xl text-slate-800 dark:text-slate-100`} /></div>
                <div className="space-y-3"><label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2"><Bot size={14} className="text-purple-500"/> DNA אישי (הנחיות AI ללקוח זה)</label><textarea value={editCrm.dnaContext} onChange={e => setEditCrm((prev:any)=>({...prev, dnaContext: e.target.value}))} rows={4} placeholder="למשל: לתת לו יחס מיוחד, תמיד להציע מנוף..." className={`w-full p-6 rounded-[2rem] text-sm font-bold outline-none border-2 transition-all ${inputBg} focus:border-purple-500 shadow-2xl text-slate-800 dark:text-slate-100 resize-none leading-relaxed`} /></div>
                <div className="grid grid-cols-2 gap-8">
                   <div className="space-y-3"><label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><UserCheck size={14}/> איש קשר</label><input value={editCrm.contactName} onChange={e => setEditCrm((prev:any)=>({...prev, contactName: e.target.value}))} className={`w-full p-6 rounded-[2rem] text-xs font-black outline-none border-2 ${inputBg} focus:border-indigo-500 shadow-xl text-slate-800 dark:text-slate-100`} /></div>
                   <div className="space-y-3"><label className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Phone size={14}/> נייד</label><input value={editCrm.contactPhone} onChange={e => setEditCrm((prev:any)=>({...prev, contactPhone: e.target.value}))} className={`w-full p-6 rounded-[2rem] text-xs font-mono font-black outline-none border-2 ${inputBg} focus:border-emerald-500 shadow-xl text-slate-800 dark:text-slate-100`} /></div>
                </div>
             </div>
                 ...
                 <div className="p-6 bg-amber-500/5 border-2 border-dashed border-amber-500/20 rounded-3xl space-y-3">
                     ...
                     <div className="flex flex-wrap gap-2">
                         {selectedCustomer.allIds.map((id: string) => (
                             <span key={id} className="text-[9px] bg-slate-800 text-slate-400 px-2 py-1 rounded-md font-mono">{id}</span>
                         ))}
                     </div>
                 </div>
             )}
