...
export default function SabanStudioFinal() {
  ...
  // Data State - מבטיחים שהכל מתחיל כרשימה ריקה ולא null
  ...
  // --- הגנה קריטית: סינון חסין לערכי null שמונע את קריסת ה-Client Side ---
  ...
  return (
    <div className={`h-screen flex overflow-hidden font-sans ${isDarkMode ? 'bg-[#020617] text-white' : 'bg-[#f8fafc] text-slate-900'}`} dir="rtl">
      ...
      <aside className="w-72 bg-[#0f172a] text-white p-6 flex flex-col shadow-2xl z-30">
        ...
        <button onClick={() => setIsDarkMode(!isDarkMode)} className="mt-auto p-4 rounded-2xl bg-white/5 flex items-center justify-between text-white border border-white/10">
      ...
      <main className="flex-1 flex overflow-hidden relative bg-white">
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {(nodes || []).map((node, i) => (
                    <div key={node.id} className="bg-slate-50 border border-slate-200 p-6 rounded-[2.5rem] group relative hover:shadow-xl transition-all">
                       <input value={node.name} onChange={(e) => { const n = [...nodes]; n[i].name = e.target.value; setNodes(n); }} className="bg-transparent font-black text-slate-900 text-lg outline-none w-full border-b border-transparent focus:border-emerald-500/30" />
                       <textarea value={node.prompt} onChange={(e) => { const n = [...nodes]; n[i].prompt = e.target.value; setNodes(n); }} className="w-full h-32 bg-white border border-slate-200 rounded-2xl p-4 mt-4 text-xs text-slate-700 outline-none focus:ring-1 focus:ring-emerald-500/20" placeholder="הנחיה למוח (מה ראמי יענה כאן?)" />
                       <button onClick={() => setNodes(nodes.filter(nd => nd.id !== node.id))} className="absolute top-4 left-4 text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 hover:bg-rose-50 rounded-full"><Trash2 size={18}/></button>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            {activeSection === 'KNOWLEDGE' && (
              <motion.div key="knowledge" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto space-y-8">
                <header className="flex justify-between items-center border-b pb-6 border-slate-100">
                  <h2 className="text-3xl font-black italic text-slate-900 uppercase">Inventory & <span className="text-emerald-500">Database</span></h2>
                  <div className="bg-slate-100 px-4 py-2 rounded-xl border border-slate-200 flex items-center gap-2">
                    <Search size={14} className="text-slate-400"/><input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-transparent border-none outline-none text-xs w-48 font-bold" placeholder="חפש מוצר או SKU..."/>
                  </div>
                </header>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredInventory.map((item) => (
                    <div key={item.id} onClick={() => { setSelectedProduct(item); setIsModalOpen(true); }} className="bg-slate-50 border border-slate-200 p-6 rounded-[2.5rem] hover:bg-white hover:shadow-2xl transition-all cursor-pointer group border-b-4 border-b-transparent hover:border-b-emerald-500">
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center text-xs font-black">
                         ...
                         <span className="text-slate-400">מלאי: {item.stock_quantity || 0}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {activeSection === 'BRAIN_SETTINGS' && (
              <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto space-y-8">
                 <h2 className="text-3xl font-black italic uppercase text-purple-600 underline decoration-purple-200 underline-offset-8">Brain <span className="text-slate-900">DNA</span></h2>
                 <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-200 shadow-inner">
                    <textarea value={globalDNA} onChange={(e) => setGlobalDNA(e.target.value)} className="w-full h-80 bg-white border border-slate-100 rounded-[2rem] p-8 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-purple-500/10 shadow-sm leading-relaxed" placeholder="הגדר את אישיות הליבה של ראמי..." />
                 </div>
              </motion.div>
            )}
           ...
           <div className="relative w-full max-w-[300px] aspect-[9/19.5] bg-[#000] rounded-[3.5rem] border-[10px] border-[#1a1a1a] shadow-2xl overflow-hidden flex flex-col">
              ...
              <div className="flex-1 flex flex-col bg-[#070707] pt-10">
                 ...
                 <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                    {chatHistory.map((m, i) => (
                      <div key={i} className={`p-3 rounded-2xl text-[11px] font-bold leading-relaxed ${m.role === 'user' ? 'bg-blue-600 mr-6 text-white' : 'bg-white/5 border border-white/5 ml-6 text-white'}`}>
                        {m.content}
                      </div>
                    ))}
                    {isTyping && <div className="text-[9px] text-emerald-500 animate-pulse font-black px-2 uppercase tracking-widest">Processing...</div>}
                 </div>
                 <div className="p-5 bg-[#111]">
                    <div className="flex gap-2 bg-white/5 rounded-full p-1 px-4 items-center border border-white/10 group focus-within:border-emerald-500/50">
                       <input value={testInput} onChange={(e) => setTestInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSimulate()} placeholder="דבר עם המוח..." className="flex-1 bg-transparent border-none outline-none text-[11px] p-2 text-white" />
                       <button onClick={handleSimulate} className="p-2 bg-emerald-500 rounded-full text-black hover:scale-110 active:scale-90 transition-all"><Send size={14}/></button>
                    </div>
                 </div>
              </div>
           </div>
        ...
        {/* --- Training Toolbox --- */}
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-[#0f172a] p-4 rounded-full flex gap-4 shadow-[0_25px_50px_rgba(0,0,0,0.5)] z-50 ring-2 ring-white/10">
           <button className="p-4 bg-white/5 rounded-full text-emerald-500 hover:bg-emerald-500 hover:text-black transition-all"><Target size={20}/></button>
           <button className="p-4 bg-white/5 rounded-full text-blue-400 hover:bg-blue-400 hover:text-white transition-all"><LinkIcon size={20}/></button>
      ...
      <AnimatePresence>
        {isModalOpen && selectedProduct && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md p-10 flex items-center justify-center">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white p-10 rounded-[3rem] w-full max-w-4xl shadow-2xl space-y-8 relative overflow-hidden border border-slate-200">
                ...
                <div className="grid grid-cols-2 gap-10">
                   <div className="space-y-4">
                      <div className="aspect-square bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden shadow-inner group">
                         {selectedProduct.image_url ? <img src={selectedProduct.image_url} className="w-full h-full object-cover transition-transform group-hover:scale-110" /> : <ImageIcon size={48} className="text-slate-200"/>}
                      </div>
                      <input value={selectedProduct.image_url} onChange={(e) => setSelectedProduct({...selectedProduct, image_url: e.target.value})} placeholder="לינק לתמונה" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold outline-none focus:border-emerald-500/50 transition-all" />
                   </div>
                   <div className="space-y-6">
                      ...
                      <input value={selectedProduct.product_name} onChange={(e) => setSelectedProduct({...selectedProduct, product_name: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl text-sm font-bold border-none focus:ring-2 focus:ring-emerald-500/20" placeholder="שם מוצר" />
                      <div className="grid grid-cols-2 gap-4">
                         <input value={selectedProduct.sku} onChange={(e) => setSelectedProduct({...selectedProduct, sku: e.target.value})} className="p-4 bg-slate-50 rounded-2xl text-sm font-bold border-none" placeholder="SKU" />
                         <input value={selectedProduct.price} onChange={(e) => setSelectedProduct({...selectedProduct, price: parseFloat(e.target.value) || 0})} className="p-4 bg-slate-50 rounded-2xl text-sm font-bold border-none" placeholder="מחיר" type="number" />
                      </div>
                      <textarea value={selectedProduct.description} onChange={(e) => setSelectedProduct({...selectedProduct, description: e.target.value})} className="w-full h-32 bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20" placeholder="תיאור למוח (כאן ה-AI לומד על המוצר)..." />
                      <button onClick={async () => { 
                        ...
                        setLoading(false);
                      }} className="w-full py-5 bg-emerald-500 text-black font-black rounded-full shadow-lg hover:scale-[1.02] active:scale-95 transition-all text-sm uppercase">שמור שינויים במלאי</button>
                   </div>
                </div>
            </motion.div>
          </motion.div>
...
function NavBtn({ icon, label, active, onClick }: any) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-black transition-all text-[11px] uppercase tracking-tighter ${active ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20 scale-[1.03]' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
