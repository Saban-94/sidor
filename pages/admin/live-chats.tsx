            ...
            {customers.filter(c => c.id.includes(search) || c.name?.includes(search)).map((customer) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-emerald-500 font-bold italic truncate uppercase">AI Agent Active</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {activeCustomer ? (
            <>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
                 {/* הודעות יגיעו מה-Monitor Pipeline */}
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center opacity-10">
              <MessageCircle size={80} className="mb-4" />
              <p className="font-black italic uppercase tracking-[0.4em]">Select Pipeline</p>
            </div>
          )}
        </main>
      </div>
