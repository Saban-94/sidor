...
// שינוי כאן: שימוש ב-Scan במקום QrCode ליתר ביטחון ב-Build
import { 
  Bot, 
  User, 
  Search, 
...
export default function LiveChatMonitor() {
  ...
  useEffect(() => {
    console.log("[Monitor] Connecting to Firestore...");
    try {
      ...
      console.error("[Monitor] Connection Fatal Error:", e);
      setIsServerOnline(false);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0B141A] text-slate-200 font-sans antialiased flex flex-col overflow-hidden">
      <Head>
      ...
      {!isServerOnline && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#0B141A]/90 backdrop-blur-md z-50 flex items-center justify-center p-8">
          <div className="max-w-xs w-full bg-white rounded-[3rem] p-8 text-center shadow-2xl border-4 border-amber-500/20">
            {/* שימוש ב-Scan כאן */}
            <Scan size={48} className="mx-auto text-amber-500 mb-4 animate-bounce" />
        ...
        <aside className="w-80 border-r border-white/5 flex flex-col bg-[#111B21]">
          <header className="p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-lg font-black italic tracking-tighter text-white leading-none">SABAN <span className="text-emerald-500">HUB</span></h1>
                <p className="text-[9px] uppercase tracking-[0.2em] text-emerald-500/50 font-bold mt-1">Live AI Monitor</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {customers.filter(c => c.id.includes(search) || (c.name && c.name.includes(search))).map((customer) => (
              <div 
                key={customer.id}
                onClick={() => setActiveCustomer(customer)}
                className={`p-4 flex items-center gap-4 cursor-pointer border-b border-white/[0.02] transition-colors ${activeCustomer?.id === customer.id ? 'bg-[#2A3942]' : 'hover:bg-[#202C33]'}`}
              >
                <div className="w-12 h-12 rounded-full bg-slate-700 flex items-center justify-center font-black text-slate-400 border border-white/10 shrink-0">
                  {customer.name ? customer.name.charAt(0) : <User size={20} />}
                </div>
          ...
          {activeCustomer ? (
            <>
              <header className="p-4 bg-[#202C33] flex items-center justify-between border-b border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-emerald-500 font-black uppercase tracking-widest">Pipeline Managed by Gemini</p>
                  </div>
                </div>
              </header>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
                 ...
                 {/* כאן יכנס המיפוי של הודעות הלקוח מה-DB */}
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center opacity-10">
              <MessageCircle size={80} className="mb-4" />
              <p className="font-black italic uppercase tracking-[0.4em] text-sm text-center">Select Client to<br/>Audit Conversations</p>
            </div>
          )}
        </main>
      </div>
