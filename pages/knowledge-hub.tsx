  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white font-sans overflow-hidden flex flex-col" dir="rtl">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-[#1E293B]/90 backdrop-blur-xl border-t border-white/10 z-[100] flex items-center justify-around px-6">
        <button onClick={() => setActiveView('hub')} className={`flex flex-col items-center gap-1 flex-1 transition-all ${activeView === 'hub' ? 'text-emerald-400' : 'text-slate-500'}`}><BookOpen size={24} /> <span className="text-[10px] font-black uppercase">מאגר</span></button>
        <button onClick={() => setActiveView('simulator')} className={`flex flex-col items-center gap-1 flex-1 transition-all ${activeView === 'simulator' ? 'text-emerald-400' : 'text-slate-500'}`}><Bot size={24} /> <span className="text-[10px] font-black uppercase">בדיקה</span></button>
        <button onClick={() => setActiveView('add')} className={`flex flex-col items-center gap-1 flex-1 transition-all ${activeView === 'add' ? 'text-emerald-400' : 'text-slate-500'}`}><PlusCircle size={24} /> <span className="text-[10px] font-black uppercase">חדש</span></button>
      </nav>
      ...
      <AnimatePresence>
        {editingItem && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex items-center justify-center p-6">
            <div className="bg-[#1E293B] w-full max-w-xl rounded-[3.5rem] p-10 space-y-6">
               <div className="flex justify-between items-center"><h2 className="text-2xl font-black italic">עריכת ידע</h2><X className="cursor-pointer" onClick={() => setEditingItem(null)} /></div>
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white font-sans overflow-hidden flex flex-col" dir="rtl">
        ...
        <AnimatePresence mode="wait">
          ...
          {activeView === 'hub' && (
            <motion.div key="hub" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 h-full overflow-y-auto space-y-6 scrollbar-hide">
              <header className="flex justify-between items-center bg-white/5 p-6 rounded-[2.5rem] border border-white/5 shadow-xl">
                <h1 className="text-3xl font-black italic uppercase tracking-tighter">Knowledge Hub</h1>
                <BookOpen className="text-emerald-500" size={24} />
              </header>
              <div className="relative group">
                <Search className="absolute right-5 top-5 text-slate-500" size={20} />
                <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="חפש נוהל..." className="w-full bg-white/5 border border-white/10 rounded-[2rem] py-5 px-6 pr-14 font-bold outline-none focus:bg-white/10 transition-all" />
              </div>
              <div className="grid grid-cols-1 gap-4 pb-10">
                {knowledge.filter(k => k.question.includes(searchTerm) || k.answer.includes(searchTerm)).map(item => (
                  <div key={item.id} className="bg-white/5 border border-white/5 p-8 rounded-[3rem] space-y-4 relative hover:bg-white/[0.08] transition-all">
                    ...
                    <div className="flex items-center gap-2 text-[9px] font-black text-amber-400 pt-2"><Star size={12}/> איכות: {calculateQuality(item.question, item.answer)}%</div>
                  </div>
                ))}
              </div>
            </motion.div>
               ...
               <div className="relative w-full max-w-[350px] h-[700px] bg-black rounded-[50px] border-[10px] border-slate-800 shadow-2xl overflow-hidden flex flex-col scale-95 md:scale-100">
                  ...
                  <div className="flex-1 flex flex-col bg-[#E4DED5] overflow-hidden mt-4">
                     <header className="h-14 bg-[#F0F2F5] border-b border-slate-200 flex items-center gap-3 px-4 shrink-0 shadow-sm">
                        <div className="w-9 h-9 bg-slate-900 rounded-full flex items-center justify-center text-emerald-400"><Bot size={18}/></div>
                        <h2 className="font-black text-sm text-slate-900 leading-tight">Saban Brain 🧠</h2>
                     </header>
                     <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-[url('https://i.postimg.cc/mD3Wf6gL/bg-chat.png')] bg-repeat">
                        ...
                        {loading && <div className="text-[10px] font-black text-emerald-600 animate-pulse uppercase mr-2 tracking-widest">מעבד...</div>}
                     </div>
                     <footer className="h-16 bg-[#F0F2F5] flex items-center gap-2 px-3 border-t">
                        <Plus size={20} className="text-slate-500"/>
                        <form onSubmit={handleChat} className="flex-1 relative">
          ...
          {activeView === 'add' && (
            <motion.div key="add" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="p-6 h-full overflow-y-auto space-y-6">
              <h1 className="text-3xl font-black italic uppercase text-center mt-6 tracking-tighter">הזרקת ידע חדש</h1>
              <div className="space-y-4 bg-white/5 p-8 rounded-[3rem] border border-white/5 shadow-2xl">
                 ...
                 <button onClick={handleSave} disabled={loading} className="w-full bg-emerald-500 text-slate-900 py-6 rounded-[2rem] font-black text-xl flex items-center justify-center gap-3 active:scale-95 transition-all">
                   <Save size={20}/> הזרק למאגר
                 </button>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {editingItem && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex items-center justify-center p-6">
            <div className="bg-[#1E293B] w-full max-w-xl rounded-[3.5rem] p-10 space-y-6 border border-white/5 shadow-2xl">
               <div className="flex justify-between items-center"><h2 className="text-2xl font-black italic">עריכת ידע</h2><X className="cursor-pointer text-slate-500" onClick={() => setEditingItem(null)} /></div>
               <div className="space-y-4">
               ...
               <button onClick={handleUpdate} className="w-full bg-emerald-500 text-slate-900 py-6 rounded-[2rem] font-black text-xl active:scale-95 transition-all">עדכן שינויים</button>
            </div>
          </div>
        )}
      </AnimatePresence>
              ...
              <div className="relative">
                <Search className="absolute right-5 top-5 text-slate-500" size={20} />
                <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="חפש נוהל..." className="w-full bg-white/5 border border-white/10 rounded-[2rem] py-5 px-6 pr-14 font-bold outline-none focus:bg-white/10" />
              </div>
              <div className="grid grid-cols-1 gap-4 pb-10">
                {knowledge.filter(k => k.question.includes(searchTerm) || k.answer.includes(searchTerm)).map(item => (
                  <div key={item.id} className="bg-white/5 border border-white/5 p-8 rounded-[3rem] space-y-4 hover:bg-white/[0.08] transition-all">
                    <div className="flex justify-between items-center">
                      <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 rounded-full text-[9px] font-black uppercase">{item.category}</span>
                      <div className="flex gap-2 text-slate-400">
                         <Edit3 size={18} className="cursor-pointer hover:text-emerald-400" onClick={() => setEditingItem(item)} />
          ...
          {activeView === 'simulator' && (
            <motion.div key="sim" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col items-center justify-center p-4 bg-[#0F172A]">
               <div className="relative w-full max-w-[350px] h-[700px] bg-black rounded-[50px] border-[10px] border-slate-800 shadow-2xl overflow-hidden flex flex-col">
                  <div className="h-12 flex items-center justify-between px-8 pt-6 text-[10px] font-bold">
                    <span>{now.toLocaleTimeString('he-IL', {hour:'2-digit', minute:'2-digit'})}</span>
                    <div className="flex gap-1.5"><Wifi size={14}/><BatteryCharging size={14} className="text-emerald-500"/></div>
                  </div>
                  <div className="flex-1 flex flex-col bg-[#E4DED5] overflow-hidden mt-4">
                     ...
                     <footer className="h-16 bg-[#F0F2F5] flex items-center gap-2 px-3 border-t">
                        ...
                        <form onSubmit={handleChat} className="flex-1 relative">
                          <input value={input} onChange={e => setInput(e.target.value)} placeholder="שאל נוהל..." className="w-full bg-white rounded-full py-2 px-4 text-xs font-bold text-slate-900 outline-none" />
                          <button type="submit" className="absolute left-1 top-1 bg-emerald-500 text-slate-900 w-7 h-7 rounded-full flex items-center justify-center"><Send size={12} className="rotate-180"/></button>
                        </form>
                     </footer>
                  </div>
               </div>
          ...
          {activeView === 'add' && (
            <motion.div key="add" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="p-6 h-full overflow-y-auto space-y-6">
              <h1 className="text-3xl font-black italic uppercase text-center mt-4">הזרקת ידע</h1>
              <div className="space-y-4 bg-white/5 p-8 rounded-[3rem] border border-white/5 shadow-xl">
                 ...
                 <textarea value={newQA.question} onChange={(e) => setNewQA({...newQA, question: e.target.value})} placeholder="השאלה..." rows={2} className="w-full p-5 rounded-2xl font-bold bg-[#1E293B] border-none outline-none resize-none" />
                 <textarea value={newQA.answer} onChange={(e) => setNewQA({...newQA, answer: e.target.value})} placeholder="התשובה..." rows={4} className="w-full p-5 rounded-2xl font-bold bg-[#1E293B] border-none outline-none resize-none" />
                 <div className="flex justify-between items-center text-xs font-black uppercase text-slate-500">
                    <span>מדד איכות</span>
                    <span className={calculateQuality(newQA.question, newQA.answer) > 70 ? 'text-emerald-500' : 'text-amber-500'}>{calculateQuality(newQA.question, newQA.answer)}%</span>
                 </div>
