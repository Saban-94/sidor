...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {import { NextApiRequest, NextApiResponse } from 'next';
import { createClient } from '@supabase/supabase-js';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    let aiText = "";
    // לולאת Fallback על ה-Model Pool
  ...
  try {
    ...
    for (const modelName of modelPool) {
      try {
        ...
        console.error(`Failed with model ${modelName}, trying next...`);
      }
    }

    const answer = aiText || "בוס, המודלים של Gemini לא זמינים כרגע.";
    return res.status(200).json({ answer });

  } catch (error) {
    console.error('API Error:', error);
    return res.status(500).json({ answer: "בוס, יש תקלה בגישה לנתונים." });
  }
}
